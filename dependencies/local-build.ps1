...
while ($true) { Get-Content -Wait -LiteralPath $a -EA 0 | Write-Output; Start-Sleep 1 }
	...
	if (Test-Path $oemYmlPath -PathType Leaf) {
		...
		if ($confXml.Version -match '^(0|[1-9]\d*)(\.(0|[1-9]\d*)){0,2}$') {
			...
			if ($oemYml.Contains($oemToReplace)) {
				$tempOemYml = $oemYml -replace $oemToReplace, $version
			} else {
				# Release archives already contain a concrete version instead of the source placeholder.
				$versionAssignmentPattern = '(?m)^(\s*\$version\s*=\s*)''[^'']*'''
				if ($oemYml -notmatch $versionAssignmentPattern) {
					throw "Couldn't find an OEM version placeholder or assignment."
				}

				$replacement = '$1' + "'$version'"
				$tempOemYml = $oemYml -replace $versionAssignmentPattern, $replacement
			}
